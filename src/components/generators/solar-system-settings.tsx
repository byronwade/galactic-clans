...
export function SolarSystemSettings({
	...
	return (
		<div className="absolute top-32 left-4 w-80 bg-slate-900/95 backdrop-blur-xl border border-orange-400/30 
			...
			<div className="p-4">
				...
				<div className="space-y-4">
					...
					<div className="space-y-3">
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Star Mass: {config.starMass.toFixed(2)} M☉
							</label>
							<input
								type="range"
								min="0.1"
								max="3.0"
								step="0.1"
								value={config.starMass}
								onChange={(e) => updateConfig({ starMass: parseFloat(e.target.value) })}
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Star Radius: {config.starRadius.toFixed(2)} R☉
							</label>
							<input
								type="range"
								min="0.1"
								max="3.0"
								step="0.1"
								value={config.starRadius}
								onChange={(e) => updateConfig({ starRadius: parseFloat(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Temperature: {config.starTemperature.toFixed(0)} K
							</label>
							<input
								type="range"
								min="2500"
								max="10000"
								step="100"
								value={config.starTemperature}
								onChange={(e) => updateConfig({ starTemperature: parseInt(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
					</div>
					...
					<div className="space-y-3">
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Planet Count: {config.planetCount}
							</label>
							<input
								type="range"
								min="1"
								max="15"
								step="1"
								value={config.planetCount}
								onChange={(e) => updateConfig({ planetCount: parseInt(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
						...
						<div className="space-y-2">
							<label className="flex items-center space-x-2 text-sm text-slate-300">
								<input
									type="checkbox"
									checked={config.hasTerrestrialPlanets}
									onChange={(e) => updateConfig({ hasTerrestrialPlanets: e.target.checked })}
									className="rounded"
								/>
							...
							<label className="flex items-center space-x-2 text-sm text-slate-300">
								<input
									type="checkbox"
									checked={config.hasGasGiants}
									onChange={(e) => updateConfig({ hasGasGiants: e.target.checked })}
									className="rounded"
								/>
							...
							<label className="flex items-center space-x-2 text-sm text-slate-300">
								<input
									type="checkbox"
									checked={config.asteroidBelt}
									onChange={(e) => updateConfig({ asteroidBelt: e.target.checked })}
									className="rounded"
								/>
							...
							<label className="flex items-center space-x-2 text-sm text-slate-300">
								<input
									type="checkbox"
									checked={config.hasRoguePlanets}
									onChange={(e) => updateConfig({ hasRoguePlanets: e.target.checked })}
									className="rounded"
								/>
					...
					<div className="space-y-3">
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								System Age: {config.systemAge.toFixed(1)} Gyr
							</label>
							<input
								type="range"
								min="0.1"
								...
								onChange={(e) => updateConfig({ systemAge: parseFloat(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Metallicity: {(config.metalicity * 100).toFixed(2)}%
							</label>
							<input
								type="range"
								min="0"
								max="0.05"
								step="0.001"
								value={config.metalicity}
								onChange={(e) => updateConfig({ metalicity: parseFloat(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
						...
						<div>
							...
							<input
								...
								max="200"
								step="10"
								value={config.cometCount}
								onChange={(e) => updateConfig({ cometCount: parseInt(e.target.value) })}
								className="w-full accent-orange-500"
							/>
						</div>
					</div>
					...
					<div className="space-y-3">
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Inner Edge: {config.habitableZone.inner.toFixed(2)} AU
							</label>
							<input
								type="range"
								min="0.1"
						...
						<div>
							<label className="block text-xs text-slate-400 mb-1">
								Outer Edge: {config.habitableZone.outer.toFixed(2)} AU
							</label>
							<input
								type="range"
								min="1.0"
